...
document.getElementById("luasPersegiPanjang").innerHTML = luasPersegiPanjang(
  5,
  3
).toString();
...
document.getElementById("hitungLingkaran").innerHTML = "\n    Diameter : "
  .concat(hitungLingkaran(5).diameter.toString(), " <br>\n    Keliling : ")
  .concat(hitungLingkaran(5).keliling.toString(), " <br> \n    Luas : ")
  .concat(hitungLingkaran(5).luas.toString(), " <br> \n");
// Sudut segitiga jika dua sudut diketahui
function sudutSegitiga(sudutA, sudutB) {
  return 180 - (sudutA + sudutB);
}
document.getElementById("sudutSegitiga").innerHTML = sudutSegitiga(
  80,
  65
).toString();
// Selisih hari antara dua tanggal
function selisihHari(tanggal1, tanggal2) {
  var selisih = Math.abs(
    new Date(tanggal2).getTime() - new Date(tanggal1).getTime()
  );
...
function inisialNama(namaLengkap) {
  return namaLengkap
    .split(" ")
    .map(function (kata) {
      return kata[0].toUpperCase();
    })
    .join("");
}
document.getElementById("inisialNama").innerHTML =
  inisialNama("John Doe").toString();
